...
const props = defineProps({ pageKey: String, title: String });
...
function reset() {
  ...
  const forms = {
    "master-project": { project_name: "", company: "", expected_start_date: today, expected_end_date: "", notes: "" },
    ...
    "master-contractor": { contractor: "", company: "", project: "", site: "", work_type: "Multiple", contractor_warehouse: "", from_date: today, to_date: "" },
  ...
  form.value = forms[props.pageKey] || {};
}
...
<template>
  <section class="cmr-modern-form cmr-master-form">
    ...
    </div><label v-if="['master-project','master-site','master-material'].includes(pageKey)" class="cmr-full-field cmr-master-notes"><span>{{ pageKey === 'master-site' ? 'Address' : pageKey === 'master-material' ? 'Description' : 'Notes' }}</span><textarea v-model="form[pageKey === 'master-site' ? 'address' : pageKey === 'master-material' ? 'description' : 'notes']" rows="3"></textarea></label><div class="cmr-bottom-actions"><button class="cmr-button" type="button" @click="emit('close')">Cancel</button><button class="cmr-button primary" type="button" :disabled="saving" @click="save">Save Master</button></div></div>
  </section>
</template>
