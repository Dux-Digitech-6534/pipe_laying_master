...
<template>
  <section class="cmr-modern-form cmr-master-form">
    <div class="cmr-form-toolbar"><div><button class="cmr-back-link" type="button" @click="emit('close')">← Back</button><h2>New {{ title }}</h2><p class="cmr-backend-document-label">CMR master setup • ERPNext backend</p></div><div class="cmr-form-actions"><button class="cmr-button" type="button" @click="emit('close')">Cancel</button><button class="cmr-button primary" type="button" :disabled="saving" @click="save">{{ saving ? 'Saving…' : 'Save' }}</button></div></div>
    <div class="cmr-native-note"><span>♢</span>This form stays inside CMR Portal and saves to the connected ERPNext master.</div>
    <div v-if="loading" class="cmr-form-message">Loading master data…</div><div v-if="error" class="cmr-form-message error">{{ error }}</div><div v-if="success" class="cmr-form-message success cmr-document-result"><b>{{ success.name }}</b> successfully created. <button type="button" @click="reset">Create another</button></div>
