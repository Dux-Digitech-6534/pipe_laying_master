...
const props = defineProps({ pageKey: String, title: String, description: String });
const records = ref([]);
const total = ref(0);
const search = ref("");
...
const ACTIVE_STATUSES = ["Approved", "Completed", "Submitted"];
...
const columns = computed(() => ({
  "pipe-register": [
    ...
    { key: "site", label: "Site" },
    { key: "laying_date", label: "Date" },
    { key: "type_of_work", label: "Type of Work" },
    { key: "contractor_assignment", label: "Contractor" },
    { key: "status", label: "Status", status: true },
  ],
  "material-movement": [
    { key: "name", label: "Document" },
    { key: "stock_entry_type", label: "Movement Type" },
    { key: "custom_cmr_site", label: "Site" },
    { key: "item_code", label: "Item Code" },
    { key: "item_name", label: "Item Name" },
    { key: "qty", label: "Quantity", number: true },
    { key: "uom", label: "UOM" },
    { key: "from_warehouse", label: "From Store" },
    { key: "to_warehouse", label: "To Store" },
  ...
  "material-stock": [
    ...
    { key: "actual_qty", label: "Quantity", number: true },
    { key: "stock_uom", label: "UOM" },
  ],
  "contractor-stock": [
    { key: "item_code", label: "Item Code" },
    { key: "item_name", label: "Item Name" },
    { key: "warehouse", label: "Contractor Store" },
    { key: "actual_qty", label: "Quantity", number: true },
    { key: "stock_uom", label: "UOM" },
  ],
  "valve-register": [
    ...
    { key: "project", label: "Project" },
    { key: "site", label: "Site" },
    { key: "installation_date", label: "Date" },
    { key: "valve_item", label: "Valve Item" },
  ...
  "restoration-register": [
    ...
    { key: "project", label: "Project" },
    { key: "site", label: "Site" },
    { key: "restoration_date", label: "Date" },
...
async function load(append = false) {
  ...
  error.value = "";
  try {
    const result = await call("cmr_pipe_laying_master.api.entries.get_report_records", {
      report_type: props.pageKey,
      search: search.value,
  ...
  <section class="cmr-master-list-page">
    ...
    <section class="cmr-master-table-card">
      <table class="cmr-master-table">
        ...
        <tbody>
          <tr v-for="(row, index) in records" :key="row.name || `${row.item_code}-${row.warehouse}`">
            <td class="cmr-row-number">{{ index + 1 }}</td>
            <td v-for="column in columns" :key="column.key">
              <span v-if="column.status" class="cmr-status-pill" :class="{ disabled: !ACTIVE_STATUSES.includes(row[column.key]) }">{{ row[column.key] || '—' }}</span>
              <span v-else :class="{ 'cmr-primary-cell': column === columns[0] }">{{ column.number ? Number(row[column.key] || 0).toFixed(2) : (row[column.key] || '—') }}</span>
            </td>
          </tr>
