...
async function load(append = false) {
  ...
  try {
    ...
    error.value = "Records load nahi hue. Permission aur connection check karein.";
  } finally {
    loading.value = false;
  }
}
...
<template>
  <section class="cmr-master-list-page">
    <PageHeader :title="title" :description="description">
      <template #actions><button class="cmr-button primary" type="button" @click="emit('create')">+ New {{ title }}</button></template>
    </PageHeader>
    ...
    <section class="cmr-master-table-card">
      <table class="cmr-master-table">
        ...
        <tbody>
          <tr v-for="(row, index) in records" :key="row.name">
            <td class="cmr-row-number">{{ index + 1 }}</td>
            <td v-for="column in columns" :key="column.key">
              <span v-if="column.status" class="cmr-status-pill" :class="{ disabled: row.disabled }">{{ row.disabled ? 'Disabled' : 'Active' }}</span>
