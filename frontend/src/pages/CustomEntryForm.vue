...
// Mobile-only (<620px): child table rows are edited one at a time in a popup sheet
// instead of an inline stacked-card table row, then shown as a compact summary card.
const isMobileView = ref(typeof window !== "undefined" && window.innerWidth < 620);
function syncMobileView() { isMobileView.value = window.innerWidth < 620; }
onMounted(() => window.addEventListener("resize", syncMobileView));
onBeforeUnmount(() => window.removeEventListener("resize", syncMobileView));
const mobileSheet = ref(null);
const mobileRow = computed(() => mobileSheet.value ? form.value[mobileSheet.value.table]?.[mobileSheet.value.index] : null);
function openMobileSheet(table, index) { mobileSheet.value = { table, index }; }
function closeMobileSheet() { mobileSheet.value = null; }
function addMobileRow(table, factory) { addRow(table, factory); openMobileSheet(table, form.value[table].length - 1); }
function addMobileBreakup() {
  const before = form.value.quantity_breakup.length;
  addBreakup();
  if (form.value.quantity_breakup.length > before) openMobileSheet("quantity_breakup", form.value.quantity_breakup.length - 1);
}
function removeMobileRow() {
  if (!mobileSheet.value) return;
  removeRow(mobileSheet.value.table, mobileSheet.value.index);
  closeMobileSheet();
}
function cardTitle(table, row) {
  if (table === "items" || table === "quantity_breakup") return row.item_code || "Select item";
  if (table === "segments") return row.pipe_no || "New segment";
  if (table === "excavation_details") return row.excavation_type || "Excavation";
  if (table === "refill_details") return row.material_type || "Refill";
  if (table === "fittings") return row.item_code || "Select item";
  return "";
}
function cardMeta(table, row) {
  if (table === "items" || table === "fittings") return [row.moc, row.pressure_rating, row.diameter].filter(Boolean).join(" · ");
  if (table === "segments") return [row.pipe_item, row.diameter].filter(Boolean).join(" · ");
  if (table === "quantity_breakup") return `${row.nos || 0} × ${row.length || 0} m`;
  return "";
}
function cardValue(table, row) {
  if (table === "items") return `${row.qty || 0} ${row.uom || ""}`.trim();
  if (table === "fittings") return `${row.quantity || 0} ${row.uom || ""}`.trim();
  if (table === "segments") return `${Number(row.actual_length || 0).toFixed(3)} m`;
  if (table === "excavation_details") return `${Number(row.actual_depth || 0).toFixed(3)} m depth`;
  if (table === "refill_details") return `${Number(row.quantity || 0).toFixed(3)} m³`;
  if (table === "quantity_breakup") return `${Number(row.total_length || 0).toFixed(3)} m`;
  return "";
}
const mobileSheetTitle = computed(() => {
  const labels = { items: "Material Item", quantity_breakup: "Pipe / Roll", segments: "Pipe Segment", excavation_details: "Excavation", refill_details: "Refill", fittings: "Fitting" };
  return mobileSheet.value ? labels[mobileSheet.value.table] || "" : "";
});
  ...
  <section class="cmr-modern-form">
    ...
    <fieldset v-if="!loading" class="cmr-document-fields" :disabled="isReadOnly">
      ...
      <div v-if="isMaterial" class="cmr-form-section">
        <div class="cmr-section-title"><span>{{ pageKey === 'material-inward' ? 3 : 2 }}</span><div><b>Material Items</b><small>Stock items and quantities</small></div><button class="cmr-button" type="button" @click="isMobileView ? addMobileRow('items', blankItem) : addRow('items', blankItem)">+ Add Item</button></div>
        <div v-if="isMobileView" class="cmr-mobile-card-list">
          <button v-for="(row,i) in form.items" :key="i" type="button" class="cmr-mobile-card" @click="openMobileSheet('items', i)"><span class="cmr-mobile-card-main"><b>{{ cardTitle('items', row) }}</b><small v-if="cardMeta('items', row)">{{ cardMeta('items', row) }}</small></span><span class="cmr-mobile-card-value">{{ cardValue('items', row) }}</span></button>
          <p v-if="!form.items.length" class="cmr-empty-inline">No items added yet. Tap + Add Item to begin.</p>
        </div>
        <div v-else class="cmr-line-table cmr-material-items-table"><table><thead><tr><th>Type</th><th>Item *</th><th>MOC</th><th>Class</th><th>Size</th><th>Qty *</th><th v-if="pageKey === 'material-inward'">Rate</th><th>UOM</th><th v-if="isTransfer">Source Stock</th><th v-if="isTransfer">Receiver Stock</th><th v-if="pageKey === 'material-inward'">Classify</th><th v-if="pageKey === 'material-inward'">Amount</th><th></th></tr></thead><tbody><tr v-for="(row, i) in form.items" :key="i">
        ...
        <template v-if="pageKey === 'material-inward'">
          ...
          <div class="cmr-section-title compact"><div><b>Pipe / Roll Quantity Breakup</b><small>Only selected Material Items classified as Pipe appear here. Nos × Length = Total Length.</small></div><button class="cmr-button" type="button" @click="isMobileView ? addMobileBreakup() : addBreakup()">+ Add Breakup</button></div>
      ...
      <template v-if="pageKey === 'pipe-laying'">
        ...
        <div class="cmr-form-section">
          ...
          <h4>Excavation</h4>
          <div v-if="isMobileView" class="cmr-mobile-card-list">
            <button v-for="(row,i) in form.excavation_details" :key="i" type="button" class="cmr-mobile-card" @click="openMobileSheet('excavation_details', i)"><span class="cmr-mobile-card-main"><b>{{ cardTitle('excavation_details', row) }}</b></span><span class="cmr-mobile-card-value">{{ cardValue('excavation_details', row) }}</span></button>
            <p v-if="!form.excavation_details.length" class="cmr-empty-inline">No excavation rows yet.</p>
          </div>
          ...
          <button class="cmr-add-inline" type="button" @click="isMobileView ? addMobileRow('excavation_details',blankExcavation) : addRow('excavation_details',blankExcavation)">+ Add excavation</button>
          ...
          <h4>Refilling</h4>
          <div v-if="isMobileView" class="cmr-mobile-card-list">
            <button v-for="(row,i) in form.refill_details" :key="i" type="button" class="cmr-mobile-card" @click="openMobileSheet('refill_details', i)"><span class="cmr-mobile-card-main"><b>{{ cardTitle('refill_details', row) }}</b></span><span class="cmr-mobile-card-value">{{ cardValue('refill_details', row) }}</span></button>
            <p v-if="!form.refill_details.length" class="cmr-empty-inline">No refill rows yet.</p>
          </div>
          ...
          <button class="cmr-add-inline" type="button" @click="isMobileView ? addMobileRow('refill_details',blankRefill) : addRow('refill_details',blankRefill)">+ Add refill</button>
    ...
    <div v-if="mobileSheet && mobileRow" class="cmr-mobile-sheet-backdrop" @click.self="closeMobileSheet">
      <div class="cmr-mobile-sheet">
        <div class="cmr-mobile-sheet-head"><b>{{ mobileSheetTitle }}</b><button type="button" class="cmr-mobile-sheet-close" @click="closeMobileSheet">×</button></div>
        <div class="cmr-mobile-sheet-body cmr-form-grid">

          <template v-if="mobileSheet.table === 'items'">
            <label><span>Type</span><select v-model="mobileRow.material_type" @change="resetItemAfterAttribute(mobileRow)"><option value="">All</option><option v-for="value in attributeValues('material_type',mobileRow)" :key="value">{{ value }}</option></select></label>
            <label><span>Item *</span><SearchSelect v-model="mobileRow.item_code" :options="filteredItems(mobileRow)" value-key="item_code" label-key="item_name" secondary-key="item_code" placeholder="Search item…" create-label="Create a new Item" @change="itemChanged(mobileRow)" @create="openMaster('master-material')" /></label>
            <label><span>MOC</span><select v-model="mobileRow.moc" :disabled="!!mobileRow.item_code" @change="resetItemAfterAttribute(mobileRow)"><option value="">All</option><option v-for="value in attributeValues('moc',mobileRow,['material_type'])" :key="value">{{ value }}</option></select></label>
            <label><span>Class</span><select v-model="mobileRow.pressure_rating" :disabled="!!mobileRow.item_code" @change="resetItemAfterAttribute(mobileRow)"><option value="">All</option><option v-for="value in attributeValues('pressure_rating',mobileRow,['material_type','moc'])" :key="value">{{ value }}</option></select></label>
            <label><span>Size</span><select v-model="mobileRow.diameter" :disabled="!!mobileRow.item_code" @change="resetItemAfterAttribute(mobileRow)"><option value="">All</option><option v-for="value in attributeValues('diameter',mobileRow,['material_type','moc','pressure_rating'])" :key="value" :value="value">{{ value }}</option></select></label>
            <label><span>Qty *</span><input v-model.number="mobileRow.qty" type="number" min="0.001" step="0.001" /></label>
            <label v-if="pageKey === 'material-inward'"><span>Rate</span><input v-model.number="mobileRow.rate" type="number" min="0" step="0.01" /></label>
            <label><span>UOM</span><input v-model="mobileRow.uom" readonly /></label>
            <label v-if="isTransfer"><span>Source Stock</span><input :value="Number(mobileRow.source_stock || 0).toFixed(3)" readonly /></label>
            <label v-if="isTransfer"><span>Receiver Stock</span><input :value="Number(mobileRow.target_stock || 0).toFixed(3)" readonly /></label>
            <label v-if="pageKey === 'material-inward'"><span>Classify</span><select v-model="mobileRow.consumable_type"><option v-for="value in selectOptions('consumable_type', ['Consumable','Non-Consumable'])" :key="value">{{ value }}</option></select></label>
            <label v-if="pageKey === 'material-inward'"><span>Amount</span><input :value="amount(mobileRow).toFixed(2)" readonly /></label>
          </template>

          <template v-else-if="mobileSheet.table === 'quantity_breakup'">
            <label><span>Pipe / Roll</span><SearchSelect v-model="mobileRow.item_code" :options="breakupItems" value-key="item_code" label-key="item_code" /></label>
            <label><span>Nos</span><input v-model.number="mobileRow.nos" type="number" min="0.001" step="0.001" @input="recalculateBreakup(mobileRow)" /></label>
            <label><span>Length</span><input v-model.number="mobileRow.length" type="number" min="0.001" step="0.001" @input="recalculateBreakup(mobileRow)" /></label>
            <label><span>Total Length</span><input :value="Number(mobileRow.total_length || 0).toFixed(3)" readonly /></label>
          </template>

          <template v-else-if="mobileSheet.table === 'segments'">
            <label><span>Pipe No *</span><input v-model="mobileRow.pipe_no" /></label>
            <template v-if="hierarchyEnabled">
              <label><span>Type</span><input value="Pipe" readonly /></label>
              <label><span>Pipe Item *</span><SearchSelect v-model="mobileRow.pipe_item" :options="filteredExecutionItems('Pipe',mobileRow)" value-key="item_code" label-key="item_name" secondary-key="item_code" placeholder="Search pipe item…" @change="itemChanged(mobileRow,'pipe_item')" /></label>
              <label><span>MOC</span><select v-model="mobileRow.moc" @change="resetExecutionItemAfterAttribute(mobileRow,'pipe_item','moc')"><option value="">All</option><option v-for="value in executionAttributeValues('Pipe','moc',mobileRow)" :key="value">{{ value }}</option></select></label>
              <label><span>Class</span><select v-model="mobileRow.pressure_rating" @change="resetExecutionItemAfterAttribute(mobileRow,'pipe_item','pressure_rating')"><option value="">All</option><option v-for="value in executionAttributeValues('Pipe','pressure_rating',mobileRow,['moc'])" :key="value">{{ value }}</option></select></label>
              <label><span>Size</span><select v-model="mobileRow.diameter" @change="resetExecutionItemAfterAttribute(mobileRow,'pipe_item','diameter')"><option value="">All</option><option v-for="value in executionAttributeValues('Pipe','diameter',mobileRow,['moc','pressure_rating'])" :key="value" :value="value">{{ value }}</option></select></label>
            </template>
            <label v-else><span>Pipe Item *</span><SearchSelect v-model="mobileRow.pipe_item" :options="options.items" value-key="item_code" label-key="item_name" secondary-key="item_code" placeholder="Search pipe item…" @change="itemChanged(mobileRow,'pipe_item')" /></label>
            <label><span>Start Node</span><input v-model="mobileRow.start_node" /></label>
            <label><span>End Node</span><input v-model="mobileRow.end_node" /></label>
            <label><span>From</span><input v-model.number="mobileRow.chainage_from" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateSegment(mobileRow)" /></label>
            <label><span>To</span><input v-model.number="mobileRow.chainage_to" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateSegment(mobileRow)" /></label>
            <label v-if="hierarchyEnabled"><span>Chainage Length</span><input :value="Number(mobileRow.chainage_length || 0).toFixed(3)" readonly /></label>
            <label><span>Actual Length *</span><input v-model.number="mobileRow.actual_length" type="number" :min="hierarchyEnabled ? 0.001 : null" step="0.001" /></label>
            <template v-if="!hierarchyEnabled">
              <label><span>Size</span><input :value="mobileRow.diameter || '—'" readonly /></label>
              <label><span>MOC</span><input :value="mobileRow.moc || '—'" readonly /></label>
              <label><span>Class</span><input :value="mobileRow.pressure_rating || '—'" readonly /></label>
            </template>
            <label><span>Contractor Stock</span><input :value="Number(mobileRow.current_stock || 0).toFixed(3)" readonly /></label>
          </template>

          <template v-else-if="mobileSheet.table === 'excavation_details'">
            <label><span>Type</span><select v-model="mobileRow.excavation_type"><option v-for="value in selectOptions('excavation_type', ['Dismantling Cement Concrete Pavement','Excavation Ordinary Soil','Excavation Hard Rock'])" :key="value">{{ value }}</option></select></label>
            <label><span>Length</span><input v-model.number="mobileRow.length" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateExcavation(mobileRow)" /></label>
            <label><span>Width</span><input v-model.number="mobileRow.width" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateExcavation(mobileRow)" /></label>
            <label><span>Calculated Depth</span><input v-model.number="mobileRow.calculated_depth" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateExcavation(mobileRow)" /></label>
            <label><span>Actual Depth</span><input v-model.number="mobileRow.actual_depth" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateExcavation(mobileRow)" /></label>
            <label v-if="hierarchyEnabled"><span>Calculated Qty (m³)</span><input :value="Number(mobileRow.calculated_qty || 0).toFixed(3)" readonly /></label>
            <label v-if="hierarchyEnabled"><span>Actual Qty (m³)</span><input :value="Number(mobileRow.actual_qty || 0).toFixed(3)" readonly /></label>
          </template>

          <template v-else-if="mobileSheet.table === 'refill_details'">
            <label><span>Material</span><select v-model="mobileRow.material_type"><option v-for="value in selectOptions('refill_material', ['Murum','Excavated Earth','Metal','Stone Dust','Sand','Good Earth','Other'])" :key="value">{{ value }}</option></select></label>
            <label><span>Length</span><input v-model.number="mobileRow.length" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateRefill(mobileRow)" /></label>
            <label><span>Width</span><input v-model.number="mobileRow.width" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateRefill(mobileRow)" /></label>
            <label><span>Depth</span><input v-model.number="mobileRow.depth" type="number" :min="hierarchyEnabled ? 0 : null" step="0.001" @input="recalculateRefill(mobileRow)" /></label>
            <label v-if="hierarchyEnabled"><span>Quantity (m³)</span><input :value="Number(mobileRow.quantity || 0).toFixed(3)" readonly /></label>
          </template>

          <template v-else-if="mobileSheet.table === 'fittings'">
            <template v-if="hierarchyEnabled">
              <label><span>Type</span><input value="Fitting" readonly /></label>
              <label><span>Fitting Item *</span><SearchSelect v-model="mobileRow.item_code" :options="filteredExecutionItems('Fitting',mobileRow)" value-key="item_code" label-key="item_name" secondary-key="item_code" placeholder="Search fitting…" @change="itemChanged(mobileRow)" /></label>
              <label><span>MOC</span><select v-model="mobileRow.moc" :disabled="!!mobileRow.item_code" @change="resetExecutionItemAfterAttribute(mobileRow,'item_code','moc')"><option value="">All</option><option v-for="value in executionAttributeValues('Fitting','moc',mobileRow)" :key="value">{{ value }}</option></select></label>
              <label><span>Class</span><select v-model="mobileRow.pressure_rating" :disabled="!!mobileRow.item_code" @change="resetExecutionItemAfterAttribute(mobileRow,'item_code','pressure_rating')"><option value="">All</option><option v-for="value in executionAttributeValues('Fitting','pressure_rating',mobileRow,['moc'])" :key="value">{{ value }}</option></select></label>
              <label><span>Size</span><select v-model="mobileRow.diameter" :disabled="!!mobileRow.item_code" @change="resetExecutionItemAfterAttribute(mobileRow,'item_code','diameter')"><option value="">All</option><option v-for="value in executionAttributeValues('Fitting','diameter',mobileRow,['moc','pressure_rating'])" :key="value" :value="value">{{ value }}</option></select></label>
            </template>
            <label v-else><span>Item *</span><SearchSelect v-model="mobileRow.item_code" :options="options.items" value-key="item_code" label-key="item_name" secondary-key="item_code" placeholder="Search fitting…" @change="itemChanged(mobileRow)" /></label>
            <label><span>Qty *</span><input v-model.number="mobileRow.quantity" type="number" :min="hierarchyEnabled ? 0.001 : null" step="0.001" /></label>
            <label><span>UOM</span><input v-model="mobileRow.uom" :readonly="hierarchyEnabled" /></label>
            <template v-if="!hierarchyEnabled">
              <label><span>Size</span><input :value="mobileRow.diameter || '—'" readonly /></label>
              <label><span>MOC</span><input :value="mobileRow.moc || '—'" readonly /></label>
              <label><span>Class</span><input :value="mobileRow.pressure_rating || '—'" readonly /></label>
            </template>
            <label><span>Stock</span><input :value="Number(mobileRow.current_stock || 0).toFixed(3)" readonly /></label>
            <template v-if="pageKey === 'pipe-laying'">
              <label><span>Junction</span><input v-model="mobileRow.junction_no" /></label>
              <label class="cmr-full-field"><span>GPS</span><button class="cmr-button" type="button" @click="captureRowGps(mobileRow)">{{ mobileRow.gps_latitude != null ? 'Refresh GPS' : 'Capture GPS' }}</button><small v-if="mobileRow.gps_latitude != null">{{ mobileRow.gps_latitude }}, {{ mobileRow.gps_longitude }}</small></label>
            </template>
          </template>

        </div>
        <div class="cmr-mobile-sheet-foot">
          <button type="button" class="cmr-button danger" @click="removeMobileRow">Remove</button>
          <button type="button" class="cmr-button primary" @click="closeMobileSheet">Done</button>
        </div>
      </div>
    </div>
  </section>
